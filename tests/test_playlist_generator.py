...
@pytest.fixture
def sample_features(n_tracks: int = 20, feature_dim: int = FEATURE_VIEW_SIZE) -> np.ndarray:
    """
...
class TestPlaylistPathfinder:
    ...
    def test_perform_pca(self, pathfinder: PlaylistPathfinder, sample_features: np.ndarray):
    ...
    def test_perform_tsne(self, pathfinder: PlaylistPathfinder, sample_features: np.ndarray):
    ...
    def test_generate_playlist_line_basic(
        ...
        assert len(playlist) == num_tracks, f"Playlist devrait avoir {num_tracks} tracks, mais a {len(playlist)}"
    ...
    def test_generate_playlist_line_length_variations(
        ...
        for num_tracks in [3, 5, 10, 15]:
            ...
            assert len(playlist) == num_tracks, (
                f"Pour num_tracks={num_tracks}, la playlist devrait avoir {num_tracks} tracks, "
                f"mais a {len(playlist)}"
            )
    ...
    def test_generate_playlist_line_same_indices(
        ...
        with pytest.raises(RuntimeError, match="Les deux tracks doivent être différentes"):
            pathfinder.generate_playlist_line(
                tracks_df=sample_tracks_df,
                raw_features=sample_features,
                track1_idx=0,
    ...
    def test_generate_playlist_line_mismatched_features(
        ...
        wrong_features = np.random.randn(10, FEATURE_VIEW_SIZE)  # 10 features au lieu de 20
    ...
    def test_generate_playlist_line_playlist_structure(
        ...
        required_keys = [
            'position', 'track_id', 'name', 'artists', 'genre', 
            'confidence', 'cosine_similarity', 'cosine_distance'
        ]
    ...
    def test_generate_playlist_line_no_duplicates(
        ...
        assert len(track_ids) == len(set(track_ids)), "Il y a des doublons dans la playlist"
    ...
    def test_generate_playlist_line_from_pca_df(
        ...
        playlist, line_points_2d, p1_2d, p2_2d = pathfinder.generate_playlist_line_from_pca_df(
    ...
    def test_analyze_playlist_quality_structure(
        ...
        required_keys = [
            'num_tracks', 'unique_genres', 'genre_diversity_ratio',
            'avg_cosine_similarity', 'avg_distance_to_line',
            'std_distance_to_line', 'max_distance_to_line',
            'avg_smoothness', 'genre_distribution'
        ]
