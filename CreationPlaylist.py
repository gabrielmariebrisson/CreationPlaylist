...
st.set_page_config(page_title="Music Playlist Generator", page_icon="🎵", layout="wide")
...
st.markdown(
    "Créez des playlists personnalisées avec l'IA - Analyse de genres musicaux par CNN"
)
...
with st.sidebar:
    ...
    spotify_client = (
        st.session_state.spotify_service.get_client()
        if st.session_state.spotify_service
        else None
    )
...
with tab2:
    ...
    st.markdown(
        _("Recherchez et analysez des morceaux directement (extraits de 30 secondes)")
    )

    col1, col2 = st.columns([3, 1])
    with col1:
        deezer_query = st.text_input(
            _("Rechercher un titre:"), placeholder="Nom de la chanson ou artiste..."
        )
    with col2:
        deezer_limit = st.slider(
            _("Nombre de résultats"), 5, 20, 10, key="deezer_slider"
        )
    ...
    if deezer_query:
        if st.button(_("🔍 Rechercher"), type="primary"):
            with st.spinner(_("Recherche en cours...")):
                ...
                deezer_results = deezer_service.search_tracks(
                    deezer_query, deezer_limit
                )
        ...
        if st.session_state.deezer_search_results:
            ...
            for i, track in enumerate(st.session_state.deezer_search_results):
                ...
                with col4:
                    ...
                    if already_analyzed:
                        st.button(
                            _("✅ Analysé"), key=f"analyzed_deezer_{i}", disabled=True
                        )
                    else:
                ...
                with col5:
                    ...
                    if already_in_playlist:
                        st.button(
                            _("✅ Ajouté"), key=f"added_playlist_{i}", disabled=True
                        )
                    else:
                        already_analyzed = any(
                            t["deezer_id"] == track.get("id")
        ...
        if valid_tracks:
            ...
            if len(valid_tracks) >= 2:
                ...
                with col2:
                    if reduction_method == "tsne":
                        perplexity = st.slider(
                            _("Perplexity (t-SNE):"), 5, 50, 30, key="tsne_perplexity"
                        )

                features_list = [
                    t["features"] for t in valid_tracks if t.get("features") is not None
                ]
                ...
                if len(features_list) >= 2:
                    ...
                    if result is not None:
                        ...
                        st.success(
                            _(
                                f"✅ {len(pca_df)} tracks prêtes pour la création de playlist"
                            )
                        )
                        st.info(
                            _(
                                "👉 Rendez-vous dans l'onglet '🎨 Playlist' pour créer votre playlist personnalisée"
                            )
                        )
                    else:
                        st.error(
                            _(f"❌ Erreur lors du calcul {reduction_method.upper()}")
                        )
                else:
            ...
            for idx, track in enumerate(valid_tracks[:20]):
                ...
                with col1:
                    st.markdown(f"**{track['name']}**")
                    st.caption(
                        f"{track['artists']} - {track['genre']} ({track['confidence']:.1%})"
                    )
...
with tab4:
    ...
    if "pca_df" not in st.session_state or st.session_state.pca_df is None:
        st.warning(
            _("⚠️ **Prérequis**: Effectuez d'abord l'analyse dans l'onglet '📊 Analyse'")
        )
        prerequisites_ok = False
    ...
    # ========================================
        ...
        if playlist_mode == "transition":
            st.markdown(
                _(
                    "Sélectionnez deux tracks pour créer une playlist progressive entre elles"
                )
            )
            ...
            num_tracks = st.slider(
                _("Nombre de tracks dans la playlist:"),
                2,
                100,
                10,
                key="num_tracks_slider",
            )

            # BOUTON DE GÉNÉRATION
            if st.button(
                _("🎯 Générer la playlist"), type="primary", key="generate_playlist_btn"
            ):
                    ...
                    with st.spinner(_("Génération de la playlist...")):
                        ...
                        try:
                            ...
                            if missing_tracks:
                                st.warning(
                                    _(
                                        f"Features non trouvées pour {len(missing_tracks)} track(s): {missing_tracks[:3]}"
                                    )
                                )
                            ...
                            if playlist is not None:
                                ...
                                st.session_state.playlist_analysis = (
                                    pathfinder.analyze_playlist_quality(playlist)
                                )
                                st.success(
                                    _(f"✅ Playlist de {len(playlist)} tracks générée!")
                                )
                            else:
        ...
        # MODE GENRE
            ...
            max_tracks = st.slider(
                _("Nombre max de tracks par genre:"), 2, 100, 20, key="genre_max_tracks"
            )

            if st.button(
                _("🎸 Générer playlist par genre"),
                type="primary",
                key="generate_genre_playlist_btn",
            ):
                if selected_genres:
                    with st.spinner(_("Génération de la playlist...")):
                        # Filtrer par genres
                        ...
                        if sort_by == "confidence":
                            genre_tracks = genre_tracks.sort_values(
                        ...
                        # Limiter le nombre
                        genre_tracks = genre_tracks.head(
                            max_tracks * len(selected_genres)
                        )
        ...
        if st.session_state.generated_playlist:
            ...
            if st.session_state.playlist_analysis:
                ...
                col3.metric(
                    _("Distance moy."), f"{analysis['avg_distance_to_line']:.3f}"
                )
                ...
                with st.expander(_("📊 Analyse détaillée")):
                    ...
                    with col2:
                        ...
                        st.write(
                            f"• {_('Distance max')}: {analysis['max_distance_to_line']:.3f}"
                        )
                        st.write(
                            f"• {_('Écart-type')}: {analysis['std_distance_to_line']:.3f}"
                        )
                        st.write(
                            f"• {_('Ratio diversité')}: {analysis['genre_diversity_ratio']:.1%}"
                        )
            ...
            sorted_playlist = sorted(
                st.session_state.generated_playlist, key=lambda x: x["position"]
            )
            ...
            for track_info in sorted_playlist:
                ...
                with col2:
                    ...
                    st.caption(
                        f"{track_info.get('artists', 'Unknown')} • {track_info['genre']}"
                    )
            ...
            deezer_tracks = (
                len(st.session_state.generated_playlist) - spotify_tracks_count
            )
            st.info(
                _(
                    f"ℹ️ {spotify_tracks_count} tracks Spotify exportables • {deezer_tracks} tracks Deezer non exportables"
                )
            )
            ...
            if spotify_tracks_count > 0:
                ...
                if st.button(
                    _("🎵 Créer la playlist sur Spotify"),
                    type="primary",
                    key="export_spotify_btn",
                ):
                    ...
                    if spotify_service:
                        ...
                        if spotify_only_tracks:
                            ...
                            if result:
                                st.markdown(
                                    f"[🎵 Ouvrir dans Spotify]({result['external_urls']['spotify']})"
                                )
                                st.balloons()
                        else:
            ...
            st.markdown("---")
            if st.button(
                _("🔄 Générer une nouvelle playlist"),
                type="secondary",
...
with tab5:
    ...
    **Enseignant référent :** Charles Brazier
    ...
    architecture_data = {
        ...
        _("Rôle Principal"): [
            _(
                "Extraction hiérarchique des caractéristiques spectrales et temporelles du spectrogramme de Mel."
            ),
            _(
                "Renforcement des caractéristiques importantes via des mécanismes d'attention."
            ),
            _(
                "Transformation en prédictions de probabilités pour les 10 genres musicaux."
            ),
        ],
    }
    ...
    **Analyse :** Les résultats montrent une excellente performance pour le genre **Blues** (100 %) et une très bonne performance pour le **Metal** (90 %). Cependant, le modèle rencontre des difficultés significatives avec le genre **Disco** (40 %), suggérant un chevauchement des caractéristiques sonores de ce genre avec d'autres, ou un besoin d'ajustement des hyperparamètres pour cette classe.
    ...
    st.image(
        PLAYLIST_IMAGE, caption=_("Génération de playlist par chemin de similarité")
    )
    st.markdown(
        _(
            """
    ...
    st.markdown(
        _(
            "Le projet a été mené sur la durée d'un **semestre universitaire**, représentant le temps de développement et de recherche principal."
        )
    )
    ...
    cost_data = {
        _("Catégorie de Coût"): [
            ...
            _("Coûts Matériels"),
            _("Coûts Logiciels"),
            _("Coûts d'Infrastructure"),
        ],
        _("Détail"): [
            _("Un semestre (recherche, codage, tests, documentation)."),
            _(
                "**Nuls**. L'entraînement du modèle a été réalisé localement sur un **MacBook M1**."
            ),
            _(
                "**Nuls**. Utilisation exclusive de bibliothèques et d'outils *open source* (ex: PyTorch, librosa)."
            ),
            _(
                "**Nuls**. Aucune utilisation de serveurs *cloud* ou de GPU dédiés n'a été nécessaire."
            ),
        ],
    }
