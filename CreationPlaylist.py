...
def get_spotify_oauth():
    """Retourne l'objet OAuth configuré pour Spotify - SANS cache fichier"""
    ...
    if not CLIENT_ID or not CLIENT_SECRET:
        st.error(_("⚠️ Variables d'environnement Spotify manquantes"))
        return None
    ...
    return SpotifyOAuth(
        ...
        scope=SPOTIFY_SCOPE,
        cache_path=None,  # ✅ CRITIQUE : Désactiver le cache fichier
        show_dialog=False,
        open_browser=False
    )
...
def get_spotify_client():
    ...
    Utilise UNIQUEMENT session_state pour isolation multi-utilisateurs
    """
    try:
        ...
        # ========================================
        # ÉTAPE 1 : Gérer le retour OAuth (code)
        # ========================================
        if "code" in query_params:
            ...
            try:
                ...
                if token_info:
                    ...
                    # Retourner le client directement (pas de rerun)
                    return spotipy.Spotify(auth=token_info['access_token'])
                else:
                    st.error(_("❌ Impossible d'obtenir le token"))
                    return None
                    
            except Exception as e:
                st.error(_(f"❌ Erreur OAuth: {e}"))
                return None
        
        # ========================================
        # ÉTAPE 2 : Vérifier session_state
        # ========================================
        if 'spotify_token' in st.session_state:
            ...
            if sp_oauth.is_token_expired(token_info):
                try:
                    # Rafraîchir le token
                    new_token_info = sp_oauth.refresh_access_token(
                        token_info['refresh_token']
                    )
                    
                    # ✅ Mettre à jour session_state avec le nouveau token
                    st.session_state.spotify_token = new_token_info
                    token_info = new_token_info
                    
                except Exception as e:
                    st.error(_(f"❌ Erreur rafraîchissement: {e}"))
                    # Token invalide, supprimer de session_state
                    del st.session_state.spotify_token
                    return None
            
            # Token valide, créer le client
            return spotipy.Spotify(auth=token_info['access_token'])
        
        # ========================================
        # ÉTAPE 3 : Pas de token = pas connecté
        # ========================================
        return None
        
    except Exception as e:
        st.error(_(f"❌ Erreur authentification: {e}"))
        return None
...
def get_user_top_tracks(spotify_client, limit=20):
    ...
    try:
        top_tracks = spotify_client.current_user_top_tracks(limit=limit, time_range='long_term')
        tracks = []
                ...
                if download_success:
                    ...
                    if genre:
                        ...
                        analyzed_track = {
                            ...
                            'deezer_id': deezer_id,
                            'uri': track.get('uri'),
                            'preview_url': preview_url,  # Stocker le preview Deezer
...
def process_track_addition(track, source='spotify'):
    ...
    try:
        ...
        # Déterminer la source correctement
...
with st.sidebar:
    ...
    if spotify_client:
        ...
        try:
            user_info = spotify_client.current_user()
            user_name = user_info.get('display_name', user_info.get('id', 'Utilisateur'))
            
            # Afficher l'info utilisateur
            st.success(_(f"✅ Connecté en tant que"))
            st.info(f"**{user_name}**")
            ...
            if st.button(_("🚪 Se déconnecter"), type="secondary", use_container_width=True):
                # Supprimer UNIQUEMENT de session_state
                if 'spotify_token' in st.session_state:
                    del st.session_state.spotify_token
                
                # Supprimer également les données utilisateur
                for key in ['spotify_top_tracks', 'spotify_saved_tracks', 'analyzed_tracks']:
                    if key in st.session_state:
                        del st.session_state[key]
                
                st.success(_("✅ Déconnexion réussie"))
                st.rerun()
                
        except Exception as e:
            st.warning(_(f"⚠️ Session expirée"))
            # Nettoyer la session
            if 'spotify_token' in st.session_state:
                del st.session_state.spotify_token
            st.rerun()
        ...
        if sp_oauth:
            ...
            **Pour commencer :**
            ...
            3. Revenez sur cet onglet
            """))
            
            # ✅ CHANGEMENT : target="_blank" pour nouvel onglet
            st.markdown(f"""
            ...
            st.error(_("❌ Configuration Spotify manquante"))
...
with tab2:
    ...
    if spotify_client:
        ...
        if st.session_state.spotify_saved_tracks:
            ...
            for i, track in enumerate(st.session_state.spotify_saved_tracks):
        ...
        if st.button(_("📊 Charger top tracks")):
            with st.spinner(_("Récupération...")):
                top_tracks = get_user_top_tracks(spotify_client, limit=limit_tracks)
        ...
        if st.session_state.spotify_top_tracks:
            ...
            for i, track in enumerate(st.session_state.spotify_top_tracks):
                col1, col2, col3, col4 = st.columns([3, 1, 1, 1])
