...
def _convert_http_exception(status: int, message: str) -> SpotifyAPIError:
    ...
    if status == 429:
        ...
        return SpotifyServerError(f"Erreur serveur Spotify ({status}): {message}", status_code=status)
...
class AsyncSpotifyService:
    ...
    def __init__(self, access_token: Optional[str] = None) -> None:
        ...
        Args:
            access_token: Token d'accès Spotify. Si None, sera récupéré depuis les variables d'environnement.
        """
    ...
    async def _refresh_access_token(self) -> str:
        ...
        if not self._refresh_token or not self._client_id or not self._client_secret:
            raise SpotifyAPIError("Credentials Spotify manquants pour le rafraîchissement")
            ...
            async with self._session.post(url, data=data, auth=auth) as response:
                if response.status == 200:
                    ...
                    if new_token:
                        ...
                        raise SpotifyAPIError("Token d'accès non présent dans la réponse")
                else:
                    error_text = await response.text()
                    raise _convert_http_exception(response.status, error_text)
        except aiohttp.ClientError as e:
            raise SpotifyAPIError(f"Erreur réseau lors du rafraîchissement: {str(e)}") from e
    ...
    async def _make_request(
        ...
        Args:
            ...
            retry_on_auth_error: Si True, réessaie avec un nouveau token en cas d'erreur 401.
                    ...
                    if response.status == 200:
                        ...
                        async with self._session.request(
                            ...
                            if retry_response.status == 200:
                                ...
                                raise _convert_http_exception(retry_response.status, error_text)
                    else:
                        error_text = await response.text()
                        raise _convert_http_exception(response.status, error_text)
            except aiohttp.ClientError as e:
    ...
    async def get_tracks_features_batch(
        ...
        for i in range(0, len(track_ids), batch_size):
            ...
            tasks = [
                self.get_track_features(track_id)
    ...
    async def get_current_user(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.exception(
                "Erreur API Spotify lors de la récupération des informations utilisateur après retries (async)",
                extra={"extra": {
                    "error_type": type(e).__name__,
                    "attempts": 3
                }}
