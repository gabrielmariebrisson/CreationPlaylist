...
class TrackProcessor:
    ...
    def process_track_analysis(
        ...
        Args:
            ...
            callback_spinner: Fonction optionnelle pour afficher un spinner (doit retourner un context manager).
        ...
        try:
            ...
            spinner_context = callback_spinner("Analyse en cours...") if callback_spinner else None
            if spinner_context:
            ...
            try:
                ...
                if track_data['type'] in ['deezer', 'search_deezer']:
                    ...
                    if not preview_url:
                        if callback_warning:
                            callback_warning("⚠️ Aucun aperçu audio disponible sur Deezer")
                        logger.warning("Aucun preview_url pour track Deezer", extra={"extra": {"track_name": track.get('name')}})
                        return False
                ...
                # Si c'est une track Spotify, rechercher sur Deezer
                    ...
                    if not preview_url:
                        if callback_error:
                            ...
                            callback_info("💡 Astuce : Essayez de rechercher directement dans l'onglet dédié")
                        logger.warning(
                            "Track Deezer non trouvée pour track Spotify",
                            extra={"extra": {"track_name": track.get('name'), "artist_name": artist_name}}
                        )
                        return False
                ...
                download_success = deezer_service.download_preview(preview_url, audio_path)
                ...
                if download_success:
                    ...
                    if result and result.get('genre'):
                        ...
                        spotify_match = None
                        if track_data['type'] in ['deezer', 'search_deezer'] and spotify_service:
                        ...
                        if spotify_match and callback_info:
                            callback_info(f"🎵 Trouvé sur Spotify: {spotify_match['name']}")
                        
                        analyzed_tracks.append(analyzed_track)
                        
                        if callback_success:
                            callback_success(f"✅ {track['name']} - {genre} ({confidence:.1%})")
                        ...
                        logger.error("Erreur lors de la prédiction du genre", extra={"extra": {"track_name": track.get('name')}})
                        return False
                else:
                    if callback_error:
                        callback_error("❌ Impossible de télécharger l'aperçu audio")
                    logger.error("Échec du téléchargement Deezer", extra={"extra": {"preview_url": preview_url}})
                    return False
            finally:
                if spinner_context:
            ...
            logger.exception(
                ...
                extra={"extra": {"track_name": track.get('name'), "error_type": type(e).__name__}}
            )
            return False
    ...
    def process_track_addition(
        ...
        try:
            ...
            if existing_analysis:
                ...
                if callback_success:
                    callback_success(f"✅ '{track['name']}' ajouté à la playlist depuis les analyses existantes.")
            ...
            if callback_success:
                callback_success(f"✅ '{track['name']}' ajouté à la playlist (non analysé).")
