...
class DeezerService:
    ...
    def search_tracks(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        Returns:
            ...
            Chaque dictionnaire contient: id, name, artists, preview_url,
            album, duration, deezer_id.
    ...
    def download_preview(self, preview_url: str, output_path: str) -> bool:
        ...
        if not preview_url:
            logger.warning(
                "URL de preview vide", extra={"extra": {"output_path": output_path}}
            )
            return False
    ...
    def find_track_from_spotify(
        ...
        try:
            ...
            for item in data.get("data", []):
                ...
                title_match = (
                    track_name.lower() in deezer_title
                    or deezer_title in track_name.lower()
                )
                artist_match = (
                    artist_name.lower() in deezer_artist
            ...
            for item in data.get("data", []):
                ...
                if preview_url:
                    deezer_id = item.get("id")
                    logger.info(
                        f"Track Deezer trouvée (premier résultat) pour "
