...
class AudioClassifier:
    """Classe pour charger le modèle PyTorch et effectuer des prédictions
    de genre musical."""
    ...
    def _load_model(self) -> None:
        ...
        try:
            logger.info(
                ...
                extra={
                    "extra": {"model_path": self.model_path, "device": str(self.device)}
                },
            )
            ...
            logger.exception(
                ...
                extra={
                    "extra": {"model_path": self.model_path, "error_type": "OSError"}
                },
            )
    ...
    def convert_song_to_matrix(
        self, audio_path: str, size: int = SPECTROGRAM_SIZE
    ) -> np.ndarray:
        """
        ...
        if not os.path.exists(audio_path):
            logger.error(
                "Fichier audio non trouvé", extra={"extra": {"audio_path": audio_path}}
            )
        ...
        try:
            ...
            logger.exception(
                ...
                extra={
                    "extra": {"audio_path": audio_path, "error_type": "NoDataError"}
                },
            )
            ...
            logger.exception(
                ...
                extra={"extra": {"audio_path": audio_path, "error_type": "OSError"}},
            )
            raise RuntimeError(f"Erreur conversion audio: {e}") from e
    ...
    def predict(
        ...
        Returns:
            Dictionnaire contenant:
                ...
                - 'probabilities': Probabilités pour tous les genres
                    (np.ndarray, optionnel).
        ...
        if self.model is None:
            ...
            confidence = np.random.uniform(
                FALLBACK_CONFIDENCE_MIN, FALLBACK_CONFIDENCE_MAX
            )
            return {
                "genre": self.LABEL_MAPPING[genre_id],
                "confidence": confidence,
                "features": (
                    None if not return_features else np.random.rand(FEATURE_DIMENSION)
                ),
                "probabilities": (
                    None if not return_probabilities else np.random.rand(NUM_GENRES)
                ),
            }
        ...
        try:
            spectrogram_tensor = (
                torch.tensor(spectrogram).unsqueeze(0).unsqueeze(0).float()
            )
            ...
            with torch.no_grad():
                ...
                all_probs = (
                    probabilities[0].cpu().numpy() if return_probabilities else None
                )
            ...
            logger.exception(
                ...
                extra={
                    "extra": {"audio_path": audio_path, "error_type": "RuntimeError"}
                },
            )
            ...
            logger.exception(
                ...
                extra={"extra": {"audio_path": audio_path, "error_type": "ValueError"}},
            )
            raise RuntimeError(f"Erreur lors de la prédiction: {e}") from e
