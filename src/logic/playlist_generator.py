...
class PlaylistPathfinder:
    ...
    def _cosine_similarity(self, vec1: np.ndarray, vec2: np.ndarray) -> float:
        ...
        Returns:
            Similarité cosinus (entre -1 et 1, mais généralement entre 0 et 1
            pour des features normalisées).
        """
    ...
    def perform_tsne(
        ...
        Args:
            ...
            random_state: Seed pour la reproductibilité.
                Par défaut: TSNE_DEFAULT_RANDOM_STATE.
            perplexity: Perplexité pour t-SNE.
                Par défaut: TSNE_DEFAULT_PERPLEXITY.

        Returns:
            Tuple (features_tsne, tsne_model, scaler) ou (None, None, None) si erreur.

        Raises:
            ValueError: Si le nombre de features est insuffisant ou si
                perplexity est invalide.
    ...
    def perform_dimensionality_reduction(
        self, features_list: List[np.ndarray], method: str = "pca", **kwargs: Any
        ...
        Args:
            ...
            **kwargs: Arguments additionnels pour t-SNE
                (perplexity, random_state, etc.).
    ...
    def generate_playlist_line(
        ...
        Utilise les embeddings bruts (1536 dimensions) pour l'interpolation
        et la recherche de voisins avec cosine similarity. La PCA est utilisée
        uniquement pour la visualisation.

        Args:
            tracks_df: DataFrame contenant les métadonnées des tracks
                (name, artists, genre, etc.).
            raw_features: Array numpy de shape (n_tracks, feature_dim)
                contenant les features brutes.
            track1_idx: Index de la track de départ.
            track2_idx: Index de la track d'arrivée.
            num_tracks: Nombre de tracks dans la playlist.
                Par défaut: DEFAULT_PLAYLIST_SIZE.
            use_pca_for_visualization: Si True, calcule les coordonnées PCA
        ...
        try:
            ...
            missing_columns = [
                col for col in required_columns if col not in tracks_df.columns
            ]
            if missing_columns:
                raise KeyError(
                    f"Colonnes manquantes dans le DataFrame: {missing_columns}"
                )
            ...
            if (
                ...
                # Transformer les features brutes en coordonnées PCA
                # pour la visualisation
                ...
                for idx, row in tracks_df.iterrows():
                    if row["track_id"] not in used_tracks:
                        ...
                        similarity = self._cosine_similarity(
                            target_feature, track_feature
                        )
                        # Convertir en distance pour trier
                        ...
                        if (
                            ...
                            if hasattr(self, "_features_pca_cache"):
                                ...
                                track_pca_point = self.pca_model.transform(
                                    feature_scaled
                                )[0]
                    ...
                    playlist_tracks.append(
                        {
                            ...
                            "distance_to_line": cosine_distance,
    ...
    def extract_raw_features_from_analyzed_tracks(
        self, pca_df: pd.DataFrame, analyzed_tracks: List[Dict[str, Any]]
    ) -> Tuple[np.ndarray, List[str]]:
        """
        Extrait les features brutes depuis analyzed_tracks et les mappe au pca_df.

        Args:
            pca_df: DataFrame contenant les métadonnées des tracks
                (doit contenir name, deezer_id, spotify_id).
            analyzed_tracks: Liste des tracks analysées avec leurs features.

        Returns:
            Tuple (raw_features_array, missing_tracks) où:
                - raw_features_array: Array numpy de shape (n_tracks,
                    feature_dim) contenant les features brutes.
                - missing_tracks: Liste des noms de tracks pour lesquelles
                    aucune feature n'a été trouvée.
        ...
        for idx, row in pca_df.iterrows():
            ...
            if feature is None:
                ...
                for t in valid_tracks:
                    if (
                        t.get("name") == name
                        or t.get("deezer_id") == deezer_id
                        or t.get("spotify_id") == spotify_id
                    ):
                        feature = t.get("features")
                        if feature is not None:
                            break
            ...
            logger.warning(
                ...
                extra={
                    "extra": {
                        ...
                        "missing_tracks": missing_features[:3],
                    }
                },
            )
    ...
    def generate_playlist_line_from_pca_df(
        ...
        Args:
            pca_df: DataFrame contenant les métadonnées
                (peut contenir PC1/PC2 pour compatibilité).
            raw_features: Array numpy de shape (n_tracks, feature_dim)
                contenant les features brutes.
            track1_idx: Index de la track de départ.
            track2_idx: Index de la track d'arrivée.
    ...
    def analyze_playlist_quality(
        ...
        try:
            ...
            for i in range(len(playlist) - 1):
                if (
                    "actual_feature" in playlist[i]
                    and "actual_feature" in playlist[i + 1]
                ):
            ...
            analysis: Dict[str, Any] = {
                ...
                "genre_diversity_ratio": (
                    unique_genres / len(playlist) if playlist else 0.0
                ),
